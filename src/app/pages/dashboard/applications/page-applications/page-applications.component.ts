...
export class PageApplicationsComponent implements OnInit {
  ...
  openForm(index: number | null = null) {
    ...
    if (index !== null) this.projectForm.patchValue(this.applications[index]);
    ...
    if (this.editingIndex !== null) {
      ...
      this.appService.update(id, data).subscribe(updated => {
        if (updated) {
          ...
          this.messageService.add({ severity: 'success', summary: 'Updated', detail: `Project "${updated.name}" updated` });
        }
      });
    } else {
      ...
      const newApp: Application = {
        ...data,
        id: BigInt(this.applications.length ? Math.max(...this.applications.map(a => Number(a.id))) + 1 : 1)
      };
      this.appService.create(newApp).subscribe(created => {
        this.applications.push(created);
        this.messageService.add({ severity: 'success', summary: 'Added', detail: `Project "${created.name}" added` });
      });
    }
    this.closeForm();
  }
    ...
    if (confirm(`Are you sure you want to delete project "${app.name}"?`)) {
      this.appService.delete(app.id).subscribe(success => {
        if (success) {
          ...
          this.messageService.add({ severity: 'success', summary: 'Deleted', detail: `Project "${app.name}" deleted` });
        }
      });
    }
  }
