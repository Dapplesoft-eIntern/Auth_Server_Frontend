...
export class PagePermissionsComponent implements OnInit {
  ...
  savePermission() {
    if (this.selectedPermission) {
      ...
      this.permissionsService.updatePermission(updated).subscribe(p => {
        ...
        this.messageService.add({ severity: 'success', summary: 'Updated', detail: `Permission ${p.code} updated` });
        this.displayDialog = false;
      });
    } else {
      ...
      this.permissionsService.addPermission(newPermission).subscribe(p => {
        ...
        this.messageService.add({ severity: 'success', summary: 'Added', detail: `Permission ${p.code} added` });
        this.displayDialog = false;
      });
    }
  }
  ...
  deletePermission(permission: Permission) {
    if (confirm(`Are you sure you want to delete permission "${permission.code}"?`)) {
      this.permissionsService.deletePermission(permission.id).subscribe(() => {
        ...
        this.messageService.add({ severity: 'success', summary: 'Deleted', detail: `Permission ${permission.code} deleted` });
      });
    }
  }
}
