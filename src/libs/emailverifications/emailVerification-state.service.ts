...
export class EmailVerificationStateService {
  ...
  loadVerifications() {
    this.dataService.getAll().subscribe(data => this.verificationsSubject.next(data));
  }
  ...
  updateVerification(id: bigint, v: EmailVerification) {
    this.dataService.update(id, v).subscribe(updated => {
      const current = this.verificationsSubject.getValue().map(item => item.id === id ? updated : item);
  ...
  deleteVerification(id: bigint) {
    this.dataService.delete(id).subscribe(() => {
      const current = this.verificationsSubject.getValue().filter(item => item.id !== id);
      this.verificationsSubject.next(current);
    });
  }
