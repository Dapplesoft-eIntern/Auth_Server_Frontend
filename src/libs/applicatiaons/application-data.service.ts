...
export class ApplicationDataService {
  ...
  private applications: Application[] = [
    { id: 1n, name: 'App One', client_id: 'client_001', client_secret: 'secret_001', redirect_uri: 'https://example.com/callback', api_base_url: 'https://api.example.com', status: 'active' },
    { id: 2n, name: 'App Two', client_id: 'client_002', client_secret: 'secret_002', status: 'inactive' },
    { id: 3n, name: 'App Three', client_id: 'client_003', client_secret: 'secret_003', redirect_uri: 'https://example.org/callback', api_base_url: 'https://api.example.org', status: 'active' }
  ];
  ...
  create(app: Application): Observable<Application> {
    const newId = this.applications.length ? Math.max(...this.applications.map(a => Number(a.id))) + 1 : 1;
