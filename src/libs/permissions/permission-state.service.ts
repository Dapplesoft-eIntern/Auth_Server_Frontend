...
export class PermissionsStateService {
  ...
  loadPermissions() {
    this.dataService.getPermissions().subscribe(data => this._permissions$.next(data));
  }
  ...
  updatePermission(permission: Permission) {
    this.dataService.updatePermission(permission).subscribe(p => {
      const updated = this._permissions$.getValue().map(item => item.id === p.id ? p : item);
      this._permissions$.next(updated);
    });
  }
  ...
  deletePermission(id: bigint) {
    this.dataService.deletePermission(id).subscribe(() => {
      const updated = this._permissions$.getValue().filter(p => p.id !== id);
      this._permissions$.next(updated);
    });
  }
}
